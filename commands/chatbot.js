...
function loadUserGroupData() {
    try {
        ...
        const raw = fs.readFileSync(USER_GROUP_DATA, 'utf8').trim();
        return raw ? JSON.parse(raw) : { groups: [], chatbot: {} };
    } catch (error) {
...
function saveUserGroupData(data) {
    try {
        ...
        // Atomic write to avoid corruption from parallel sessions writing
        // to the same shared file at the same time.
        const tmpPath = `${USER_GROUP_DATA}.${process.pid}.tmp`;
        fs.writeFileSync(tmpPath, JSON.stringify(data, null, 2));
        fs.renameSync(tmpPath, USER_GROUP_DATA);
    } catch (error) {
