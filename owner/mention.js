...
function loadState() {
	try {
        ...
        // Always default to sticktag.webp if no custom asset set
        if (!state.assetPath || state.assetPath.endsWith('mention_default.webp')) {
            return { enabled: state.enabled !== false, assetPath: 'assets/sticktag.webp', type: 'image' };
        }
        return state;
	} catch {
        // Default: enabled with sticktag.webp
        return { enabled: true, assetPath: 'assets/sticktag.webp', type: 'image' };
	}
}
...
// ─── STATUS MENTION HANDLER ─────────────────────────────────────────────────
// Jab koi status pe bot ko mention kare to sticktag.webp send ho
async function handleStatusMention(sock, message) {
    try {
        if (message.key?.fromMe) return;
        ...
        // Extract mentionedJid from all contexts
        const msg = message.message || {};
        const contexts = [
            msg.extendedTextMessage?.contextInfo,
            msg.imageMessage?.contextInfo,
        ...
        // Bot mentioned in status — reply to the status poster with sticktag.webp
        const senderJid = message.key?.participant || message.participant || remoteJid;
        if (!senderJid || senderJid === 'status@broadcast') return;

        const imgPath = require('path').join(__dirname, '..', 'assets', 'sticktag.webp');
        const fs2 = require('fs');
        if (fs2.existsSync(imgPath)) {
            await sock.sendMessage(senderJid, {
                image: fs2.readFileSync(imgPath),
                caption: '⚠️ Tag mat kar!'
            });
        }
    } catch (err) {
        console.error('[StatusMention] Error:', err.message);
    }
}

module.exports = { handleMentionDetection, mentionToggleCommand, setMentionCommand, handleStatusMention };
